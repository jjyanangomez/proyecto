...
class ReportModel{
    public function ListReportes($id, $codigo){
        ...
        $resSQL=$miconexion->consulta("SELECT re.IdReporte AS 'Identificador', re.Nombre, re.Correo, re.ResulEvaluacion As Calificacion, DATE_FORMAT(re.FechaInicio,'%Y/%m/%d') AS FechaInicio, TIMEDIFF(TIME_FORMAT(re.FechaFinal,'%H:%i:%S'),TIME_FORMAT(re.FechaInicio,'%H:%i:%S')) AS 'Duracion', bp.Tema FROM reportes re, cuestionarios cu, bancopreguntas bp WHERE cu.IdBanco = bp.IdBanco AND cu.CodigoCuestionario= re.CodeCuestionario AND cu.IdCuestionario ='$id' AND cu.CodigoCuestionario = '$codigo'");
